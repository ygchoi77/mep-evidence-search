...
동의해야 호출 버튼을 사용할 수 있습니다. 화면에는 OpenAI 공식 표준 요금에 따른 질문별·현재
접속 세션별 예상 달러 비용을 우선 표시하고, 토큰 수는 질문별 계산 근거로 함께 표시합니다.
예상액에는 모델 입력·캐시·출력 토큰과 File Search 호출료가 포함되며, Vector Store 보관료와
Cloudflare 비용은 포함되지 않습니다. 실제 청구액은 OpenAI 사용량 대시보드를 기준으로
확인해야 합니다.
